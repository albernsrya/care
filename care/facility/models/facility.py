...
class FacilityLocalGovtBody(models.Model):
    ...
    def __str__(self):
        return (f"{getattr(self.local_body, 'name', '-')} "
...
class HospitalDoctors(FacilityBaseModel, FacilityRelatedPermissionMixin):
    ...
    class Meta:
        indexes = [
            PartialIndex(fields=["facility", "area"],
                         unique=True,
    ...
    CSV_MAKE_PRETTY = {
        "hospitaldoctors__area": (lambda x: REVERSE_DOCTOR_TYPES[x])
    }
...
class Building(FacilityBaseModel):
    ...
    num_rooms = models.IntegerField(validators=[MinValueValidator(0)],
                                    default=0)
    num_floors = models.IntegerField(validators=[MinValueValidator(0)],
                                     default=0)
    num_buildings = models.IntegerField(validators=[MinValueValidator(0)],
                                        default=0)  # For Internal Use only
...
class Room(FacilityBaseModel):
    ...
    beds_capacity = models.IntegerField(validators=[MinValueValidator(0)],
                                        default=0)
    occupied_beds = models.IntegerField(validators=[MinValueValidator(0)],
                                        default=0)
    room_type = models.IntegerField(choices=ROOM_TYPES)
...
class InventoryItem(FacilityBaseModel):
    ...
    def __str__(self):
        return (self.name + " with unit " + self.unit +
                " with minimum stock " + str(self.minimum_stock))
...
class Inventory(FacilityBaseModel):
    ...
    quantitiy = models.IntegerField(validators=[MinValueValidator(0)],
                                    default=0)

    def __str__(self):
        return (self.item.name + " : " + str(self.quantitiy) + " " +
                self.item.unit + " in " + str(self.facility))
...
class InventoryLog(FacilityBaseModel):
    ...
    updated_by = models.ForeignKey(User,
                                   ...
                                   null=True,
                                   blank=True)
    prev_count = models.IntegerField(validators=[MinValueValidator(0)],
                                     default=0)
    new_count = models.IntegerField(validators=[MinValueValidator(0)],
                                    default=0)

    def __str__(self):
        return ("Item " + str(self.inventory) + " Updated from " +
                str(self.prev_count) + " to " + str(self.new_count) +
                " updated by " + str(self.updated_by))
