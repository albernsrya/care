...
from care.facility.models import READ_ONLY_USER_TYPES, FacilityBaseModel, pretty_boolean, reverse_choices
...
class ResourceRequest(FacilityBaseModel):
    ...
    status = models.IntegerField(choices=RESOURCE_STATUS_CHOICES,
                                 default=10,
                                 null=False,
                                 blank=False)
    category = models.IntegerField(choices=RESOURCE_CATEGORY_CHOICES,
                                   default=100,
                                   null=False,
                                   blank=False)
    sub_category = models.IntegerField(choices=RESOURCE_SUB_CATEGORY_CHOICES,
                                       default=1000,
                                       null=False,
                                       blank=False)
...
class ResourceRequestComment(FacilityBaseModel):
    request = models.ForeignKey(ResourceRequest,
                                on_delete=models.PROTECT,
                                null=False,
                                blank=False)
    created_by = models.ForeignKey(
