...
class FacilityInventoryUnitConverter(models.Model):
    ...
    def __str__(self):
        return ("1 " + str(self.to_unit) + " x " + str(self.multiplier) +
                " =  1" + str(self.from_unit))
...
class FacilityInventoryItem(models.Model):
    ...
    allowed_units = models.ManyToManyField(FacilityInventoryUnit,
...
class FacilityInventoryLog(FacilityBaseModel, FacilityRelatedPermissionMixin):
    ...
    facility = models.ForeignKey("Facility",
                                 ...
                                 null=False,
                                 blank=False)
    item = models.ForeignKey(FacilityInventoryItem,
                             on_delete=models.SET_NULL,
                             null=True,
                             blank=False)
    ...
    unit = models.ForeignKey(FacilityInventoryUnit,
                             on_delete=models.SET_NULL,
                             null=True,
                             blank=False)
...
class FacilityInventorySummary(FacilityBaseModel,
    ...
    quantity = models.FloatField(
        default=0)  # Automatically Set // NOT EDITABLE BY ADMIN
    is_low = models.BooleanField(default=False)

    class Meta:
        indexes = [
            PartialIndex(fields=["facility", "item"],
                         unique=True,
                         where=PQ(deleted=False))
        ]


class FacilityInventoryMinQuantity(FacilityBaseModel,
                                   FacilityRelatedPermissionMixin):
    """
...
class FacilityInventoryBurnRate(FacilityBaseModel,
                                FacilityRelatedPermissionMixin):
    """
