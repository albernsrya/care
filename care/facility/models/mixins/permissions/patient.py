...
class PatientPermissionMixin(BasePermissionMixin):
    ...
    def has_object_read_permission(self, request):
        ...
        return request.user.is_superuser or (
            ...
            or (self.facility and request.user in self.facility.users.all()
                or doctor_allowed) or
            (request.user.user_type >= User.TYPE_VALUE_MAP["DistrictLabAdmin"]
             and (request.user.district == self.district or
                  (self.facility
                   and request.user.district == self.facility.district))) or
            (request.user.user_type >= User.TYPE_VALUE_MAP["StateLabAdmin"] and
             (request.user.state == self.state or
              (self.facility and request.user.state == self.facility.state))))
    ...
    def has_object_write_permission(self, request):
        ...
        return request.user.is_superuser or (
            (hasattr(self, "created_by") and request.user == self.created_by)
            or (doctor_allowed) or
            (self.facility and request.user in self.facility.users.all()) or
            (request.user.user_type >= User.TYPE_VALUE_MAP["DistrictLabAdmin"]
             and (request.user.district == self.district or
                  (self.facility
                   and request.user.district == self.facility.district))) or
    ...
    def has_object_transfer_permission(self, request):
        ...
        new_facility = Facility.objects.filter(
            id=request.data.get("facility", None)).first()
...
class PatientRelatedPermissionMixin(BasePermissionMixin):
    ...
    def has_object_read_permission(self, request):
        return (
            ...
            or (self.patient.facility
                and request.user in self.patient.facility.users.all())
            or (self.assigned_to == request.user) or
            (request.user.user_type >= User.TYPE_VALUE_MAP["DistrictLabAdmin"]
