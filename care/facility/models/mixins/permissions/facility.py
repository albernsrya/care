...
class FacilityPermissionMixin(BasePermissionMixin):
    ...
    def has_object_read_permission(self, request):
        return (
            ...
            (hasattr(self, "district") and
             request.user.user_type >= User.TYPE_VALUE_MAP["DistrictLabAdmin"]
             and request.user.district == self.district) or
            (hasattr(self, "state")
    ...
    def has_object_write_permission(self, request):
        if (request.user.user_type
            ...
            return False
        if request.user.user_type < User.TYPE_VALUE_MAP[
                "Staff"]:  # todo Temporary
            return False
        return self.has_object_read_permission(request)

    def has_object_update_permission(self, request):
        return super().has_object_update_permission(
            request) or self.has_object_write_permission(request)
...
class FacilityRelatedPermissionMixin(BasePermissionMixin):
    ...
    def has_write_permission(request):
        ...
        return (request.user.is_superuser or request.user.verified) and (
            (hasattr(facility, "created_by")
             and request.user == facility.created_by) or
            (hasattr(facility, "district") and
             request.user.user_type >= User.TYPE_VALUE_MAP["DistrictLabAdmin"]
             and request.user.district == facility.district) or
            (hasattr(facility, "state")
             and request.user.user_type >= User.TYPE_VALUE_MAP["StateLabAdmin"]
             and request.user.state == facility.state) or
            (request.user in facility.users.all()))
