...
class TestPatientStatsHistory(TestBase):
    ...
    def test_create_patient_history(self):
        ...
        self.assertDictEqual(
            response.json(),
            self.get_detail_representation(stats_data, facility=facility),
        )
        ...
        self.assertDictEqual(
            response.json(),
            self.get_detail_representation(new_stats_data, facility=facility),
        )
    ...
    def test_patient_history_list(self):
        ...
        facility_2 = self.create_facility(district=self.district,
                                          name="Facility 2")

        FacilityPatientStatsHistory.objects.create(facility=facility,
        ...
        FacilityPatientStatsHistory.objects.create(facility=facility,
                                                   entry_date=datetime.date(
                                                       2020, 4, 2),
                                                   **stats_data)
        FacilityPatientStatsHistory.objects.create(facility=facility_2,
                                                   entry_date=datetime.date(
                                                       2020, 4, 2),
                                                   **stats_data)
                ...
                "results": [
                    {
                        ...
                        "entry_date":
                        datetime.date(2020, 4, 2).strftime("%Y-%m-%d"),
                        "created_date":
                        mock_equal,
                        "modified_date":
                        mock_equal,
                    ...
                    },
                    {
                        ...
                        str(facility.external_id),
                        "entry_date":
                        datetime.date(2020, 4, 1).strftime("%Y-%m-%d"),
                        "created_date":
                        mock_equal,
                        "modified_date":
                        mock_equal,
    ...
    def test_retrieve_url_by_location(self):
        ...
        obj = FacilityPatientStatsHistory.objects.create(
            facility=facility,
            entry_date=datetime.date(2020, 4, 1),
            **stats_data)
