...
class TestPatient(TestBase):
    ...
    def get_list_representation(self, patient=None):
        ...
        return {
            ...
            facility_id,
            "facility_object":
            self.get_facility_representation(patient.facility),
            "blood_group":
            patient.blood_group,
            "allow_transfer":
            ...
            mock_equal,
            "source":
            patient.get_source_display(),
            "nearest_facility":
            getattr(patient.nearest_facility, "id", None),
            **self.get_local_body_district_state_representation(patient),
            "date_of_receipt_of_information":
            patient.date_of_receipt_of_information,
        }
    ...
    def _get_contact_patients_representation(self,
                                             patient: PatientRegistration):
        return [{
            "patient_in_contact":
            contacted_patient.patient_in_contact,
            "patient_in_contact_object":
            self.get_list_representation(contacted_patient.patient_in_contact),
            "relation_with_patient":
            contacted_patient.get_relation_with_patient_display(),
            "mode_of_contact":
            contacted_patient.get_mode_of_contact_display(),
            "date_of_first_contact":
            contacted_patient.date_of_first_contact,
            "date_of_last_contact":
            contacted_patient.date_of_last_contact,
            "is_primary":
            contacted_patient.is_primary,
            "condition_of_contact_is_symptomatic":
            contacted_patient.condition_of_contact_is_symptomatic,
    ...
    def get_detail_representation(self, patient: Any):
        ...
        return {
            ...
            patient.number_of_chronic_diseased_dependents,
            "medical_history":
            self._get_medical_history_representation(medical_history),
            ...
            **self.get_local_body_district_state_representation(patient),
            "nearest_facility":
            patient.nearest_facility,
            "nearest_facility_object":
            self.get_facility_representation(patient.nearest_facility),
            "meta_info":
            self._get_metainfo_representation(patient),
            "contacted_patients":
            self._get_contact_patients_representation(patient),
            "date_of_receipt_of_information":
            mock_equal,
        }
    ...
    def test_patient_creation(self):
        ...
        self.assertEqual(response.status_code,
                         status.HTTP_403_FORBIDDEN)  # only for verified users
        ...
        self.assertDictEqual(response.json(),
                             self.get_detail_representation(patient_data))

        patient = PatientRegistration.objects.filter(
            countries_travelled=patient_data["countries_travelled"]).first()
        self.assertIsNotNone(patient)
        self.assertIsNotNone(patient.medical_history.all().count(),
                             len(patient_data["medical_history"]))

        self.assertIsNotNone(patient.patient_search_id)
        patient_search = PatientSearch.objects.get(
            pk=patient.patient_search_id)
        self.assertIsNotNone(patient_search)
        self.assertEqual(patient_search.phone_number,
    ...
    def test_patient_retrieval(self):
        ...
        self.assertDictEqual(response.json(),
                             self.get_detail_representation(patient))
    ...
    def test_patient_patch(self):
        ...
        response = self.client.patch(
            self.get_url(str(patient.external_id)),
        ...
        self.assertEqual(response.status_code,
                         status.HTTP_200_OK)  # only for verified users
    ...
    def test_patient_put(self):
        ...
        patient.contacted_patients.add(
            PatientContactDetails(
                **{
                    ...
                    "mode_of_contact":
                    PatientContactDetails.ModeOfContactEnum.CLEANED_USED_ITEMS.
                    value,
                    "date_of_last_contact":
        ...
        data.update({
            "disease_status":
            new_disease_status.value,
            "contacted_patients": [{
                "patient_in_contact":
                str(self.patient.external_id),
                "relation_with_patient":
                PatientContactDetails.RelationEnum.FRIEND.name,
                "mode_of_contact":
                PatientContactDetails.ModeOfContactEnum.CO_PASSENGER_AEROPLANE.
                name,
                "date_of_last_contact":
                "2020-04-10",
                "is_primary":
        ...
        keys_to_pop = [
            key for key, value in data.items() if value is mock_equal
        ] + [
            "meta_info",
            "nationality",
            "passport_no",
        ...
        response = self.client.put(self.get_url(str(patient.external_id)),
                                   data,
                                   format="json")
        self.assertEqual(response.status_code,
                         status.HTTP_200_OK)  # only for verified users
        ...
        self.assertEquals(
            contacted_patient.relation_with_patient,
            PatientContactDetails.RelationEnum.FRIEND.value,
        )
        self.assertEquals(
            contacted_patient.mode_of_contact,
            PatientContactDetails.ModeOfContactEnum.CO_PASSENGER_AEROPLANE.
            value,
        )
        self.assertEquals(contacted_patient.date_of_last_contact,
    ...
    def test_patient_list_retrieval(self):
        ...
        self.assertDictEqual(
            response.json(),
            {
                ...
                "next": None,
                "previous": None,
                "results": [self.get_list_representation(patient_2)],
            },
        )
        ...
        self.assertDictEqual(
            response.json(),
            {
                ...
                "next": None,
                "previous": None,
                "results": [self.get_list_representation(patient)],
            },
        )
    ...
    def test_superuser_access(self):
        ...
        response = self.client.get(
            self.get_url(entry_id=str(patient.external_id)))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertDictEqual(response.json(),
                             self.get_detail_representation(patient))
    ...
    def test_search(self):
        ...
        self.client.force_authenticate(user)
        response = self.client.get(
            f"{self.get_url()}search/?phone_number={self.patient.phone_number}"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_patient_transfer(self):
        ...
        new_facility_user.username = (
            f"{new_facility_user.username}_test_patient_transfer")
        ...
        response = self.client.post(
            ...
            {
                "facility": str(new_facility.external_id),
