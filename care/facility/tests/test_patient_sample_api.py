...
class TestPatientSampleApi(TestBase):
    ...
    def get_list_representation(self, obj: PatientSample) -> dict:
        return {
            ...
            obj.patient.name,
            "patient_has_sari":
            obj.patient.has_SARI,
            "patient_has_confirmed_contact":
            obj.patient.contact_with_confirmed_carrier,
            "patient_has_suspected_contact":
            obj.patient.contact_with_suspected_carrier,
            "patient_travel_history":
            obj.patient.countries_travelled,
            "facility":
            str(obj.consultation.facility.external_id),
            "facility_object":
            self.get_facility_representation(obj.consultation.facility),
            "sample_type":
            ...
            "patient":
            str(obj.patient.external_id),
            "consultation":
            str(obj.consultation.external_id),
            "date_of_sample":
            obj.date_of_sample,
            "date_of_result":
            obj.date_of_result,
            "sample_type_other":
            obj.sample_type_other,
            "has_sari":
            obj.has_sari,
            "has_ari":
            obj.has_ari,
            ...
            obj.diff_diagnosis,
            "etiology_identified":
            obj.etiology_identified,
            "is_atypical_presentation":
            obj.is_atypical_presentation,
            "atypical_presentation":
            obj.atypical_presentation,
            "is_unusual_course":
            obj.is_unusual_course,
            "fast_track":
            obj.fast_track,
        }
    ...
    def test_create_sample_api(self):
        ...
        self.assertEqual(sample.result,
                         PatientSample.SAMPLE_TEST_RESULT_MAP["AWAITING"])
    ...
    def test_sample_flow(self):
        ...
        with freeze_time("2020-04-01"):
            ...
            response = self.client.patch(
                self.get_url(str(sample.external_id)),
                {"status": "APPROVED"},
                format="json",
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            sample.refresh_from_db()
            self.assertEquals(sample.status,
                              PatientSample.SAMPLE_TEST_FLOW_MAP["APPROVED"])
        ...
        with freeze_time("2020-04-02"):
            response = self.client.patch(
                self.get_url(str(sample.external_id)),
                {"status": "SENT_TO_COLLECTON_CENTRE"},
                format="json",
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            sample.refresh_from_db()
            self.assertEquals(
                sample.status,
                PatientSample.SAMPLE_TEST_FLOW_MAP["SENT_TO_COLLECTON_CENTRE"],
            )
            self.assertEquals(sample.result,
                              PatientSample.SAMPLE_TEST_RESULT_MAP["AWAITING"])
            self.assertEquals(
                sample.date_of_sample.date(),
                make_aware(datetime.datetime(2020, 4, 2)).date(),
            )
            self.assertIsNone(sample.date_of_result)
        ...
        with freeze_time("2020-04-03"):
            ...
            self.assertEquals(
                sample.status,
                PatientSample.SAMPLE_TEST_FLOW_MAP["RECEIVED_AND_FORWARED"],
            )
            self.assertEquals(sample.result,
                              PatientSample.SAMPLE_TEST_RESULT_MAP["AWAITING"])
            self.assertEquals(
        ...
        with freeze_time("2020-04-04"):
            ...
            self.assertEquals(
                sample.status,
                PatientSample.SAMPLE_TEST_FLOW_MAP["RECEIVED_AT_LAB"])
            self.assertEquals(sample.result,
                              PatientSample.SAMPLE_TEST_RESULT_MAP["AWAITING"])
            self.assertEquals(
                sample.date_of_sample.date(),
        ...
        with freeze_time("2020-04-05"):
            ...
            self.assertEquals(sample.result,
                              PatientSample.SAMPLE_TEST_RESULT_MAP["NEGATIVE"])
            self.assertEquals(
                sample.date_of_sample.date(),
                make_aware(datetime.datetime(2020, 4, 2)).date(),
            )
            self.assertEquals(
                sample.date_of_result.date(),
                make_aware(datetime.datetime(2020, 4, 5)).date(),
            )
