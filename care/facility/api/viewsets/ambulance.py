...
class AmbulanceFilterSet(filters.FilterSet):
    ...
    third_district_name = filters.CharFilter(field_name="third_district__name",
...
class AmbulanceViewSet(
    ...
    def add_driver(self, request):
        ...
        driver = ambulance.ambulancedriver_set.create(
            **serializer.validated_data)
        return Response(data=AmbulanceDriverSerializer(driver).data,
    ...
    def remove_driver(self, request):
        ...
        driver = ambulance.ambulancedriver_set.filter(
            id=serializer.validated_data["driver_id"]).first()
        if not driver:
            raise serializers.ValidationError(
                {"driver_id": "Detail not found"})
