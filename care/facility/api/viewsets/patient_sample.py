...
from care.facility.api.serializers.patient_sample import (PatientSampleDetailSerializer, PatientSamplePatchSerializer,
    ...
    def filter_queryset(self, request, queryset, view):
        if request.user.is_superuser:
            ...
            if request.user.user_type >= User.TYPE_VALUE_MAP["StateLabAdmin"]:
                q_objects |= Q(
                    consultation__facility__state=request.user.state)
            elif request.user.user_type >= User.TYPE_VALUE_MAP[
                    "DistrictLabAdmin"]:
                q_objects |= Q(
                    consultation__facility__district=request.user.district)
...
class PatientSampleViewSet(
    ...
    queryset = (PatientSample.objects.all().select_related(
        "consultation",
        "patient",
    ...
    def perform_create(self, serializer):
        ...
        if not validated_data.get("patient") and not validated_data.get(
                "consultation"):
            raise ValidationError({
                "non_field_errors":
                ["Either of patient or consultation is required"]
            })

        if "consultation" not in validated_data:
            validated_data[
                "consultation"] = PatientConsultation.objects.filter(
                    patient=validated_data["patient"]).last()
            if not validated_data["consultation"]:
                raise ValidationError(
                    {"patient": ["Invalid id/ No consultation done"]})

        with transaction.atomic():
            notes = validated_data.pop("notes", "created")
            instance = serializer.create(validated_data)
            instance.patientsampleflow_set.create(status=instance.status,
                                                  notes=notes,
