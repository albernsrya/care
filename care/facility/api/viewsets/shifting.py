...
from care.facility.api.serializers.shifting import (ShiftingDetailSerializer, ShiftingRequestCommentSerializer,
                                                    ShiftingSerializer, has_facility_permission)
from care.facility.models import (BREATHLESSNESS_CHOICES, SHIFTING_STATUS_CHOICES, PatientConsultation, ShiftingRequest,
                                  ShiftingRequestComment, User)
...
class ShifitngRequestCommentViewSet(
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_superuser:
            ...
            if self.request.user.user_type >= User.TYPE_VALUE_MAP[
                ...
                q_objects |= Q(request__shifting_approving_facility__state=self
                               .request.user.state)
                q_objects |= Q(
                ...
                q_objects = Q(request__orgin_facility__district=self.request.
                              user.district)
                q_objects |= Q(
                    request__shifting_approving_facility__district=self.
            ...
            q_objects |= Q(
                request__shifting_approving_facility__id__in=facility_ids)
    ...
    def get_request(self):
        ...
        queryset = queryset.filter(
            external_id=self.kwargs.get("shift_external_id"))
