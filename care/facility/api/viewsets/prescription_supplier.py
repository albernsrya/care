...
class PrescriptionSupplierConsultationViewSet(mixins.ListModelMixin,
    ...
    def get_queryset(self):
        ...
        return self.queryset.filter(
            ...
            | Q(facility__users__id__exact=self.request.user.id)).distinct(
                "id")


class PrescriptionSupplierFilter(filters.FilterSet):
    patient = filters.CharFilter(
        field_name="consultation__patient__external_id")
...
class PrescriptionSupplierViewSet(
    ...
    queryset = (PrescriptionSupplier.objects.all().select_related(
        "facility", "consultation", "consultation__patient").order_by("-id"))
    filter_backends = (filters.DjangoFilterBackend, )
    ...
    def get_queryset(self):
        if self.request.user.is_superuser:
            ...
            return self.queryset.filter(
                ...
                "StateLabAdmin"]:
            return self.queryset.filter(
                facility__state=self.request.user.state)
        return self.queryset.filter(
            facility__users__id__exact=self.request.user.id)
