...
from care.facility.api.serializers.patient_investigation import (InvestigationValueCreateSerializer,
...
class InvestigationGroupViewset(mixins.ListModelMixin,
...
class PatientInvestigationViewSet(mixins.ListModelMixin,
                                  mixins.RetrieveModelMixin,
                                  viewsets.GenericViewSet):
...
class PatientInvestigationSummaryViewSet(mixins.ListModelMixin,
                                         mixins.RetrieveModelMixin,
                                         viewsets.GenericViewSet):
...
class InvestigationValueViewSet(
    ...
    def get_queryset(self):
        queryset = self.queryset.filter(consultation__external_id=self.kwargs.
                                        get("consultation_external_id"))
        if self.request.user.is_superuser:
            return queryset
        elif self.request.user.user_type >= User.TYPE_VALUE_MAP[
                "StateLabAdmin"]:
            ...
            return queryset.filter(
                consultation__patient__facility__district=self.request.user.
                district)
        filters = Q(consultation__patient__facility__users__id__exact=self.
                    request.user.id)
        filters |= Q(consultation__assigned_to=self.request.user)
    ...
    def get_sessions(self, request, *args, **kwargs):
        ...
        return Response(
            list(
                queryset.distinct("session").annotate(
    ...
    def batchUpdate(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            for investigation in investigations:
                ...
                obj = queryset.filter(
                    external_id=investigation["external_id"]).first()
                if not obj:
                    raise ValidationError(
                        {investigation["external_id"]: "not found"})
