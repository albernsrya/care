...
from care.facility.models import (BREATHLESSNESS_CHOICES, FACILITY_TYPES, SHIFTING_STATUS_CHOICES, VEHICLE_CHOICES,
                                  Facility, PatientRegistration, ShiftingRequest, ShiftingRequestComment, User)
...
def has_facility_permission(user, facility):
    if not facility:
        return False
    return (user.is_superuser or (facility and user in facility.users.all())
            or (user.user_type >= User.TYPE_VALUE_MAP["DistrictLabAdmin"] and
