...
from care.facility.models import (DISEASE_CHOICES, GENDER_CHOICES, Disease, Facility, FacilityPatientStatsHistory,
                                  PatientContactDetails, PatientMetaInfo, PatientNotes, PatientRegistration,
                                  PatientSearch)
...
class PatientContactDetailsSerializer(serializers.ModelSerializer):
    relation_with_patient = ChoiceField(
        choices=PatientContactDetails.RelationChoices)
    mode_of_contact = ChoiceField(
        choices=PatientContactDetails.ModeOfContactChoices)

    patient_in_contact_object = PatientListSerializer(
        read_only=True, source="patient_in_contact")
    patient_in_contact = serializers.UUIDField(
        source="patient_in_contact.external_id")
...
class PatientDetailSerializer(PatientListSerializer):
    ...
    def validate(self, attrs):
        ...
        if (not self.partial and not validated.get("age")
