    ...
    def update(self, instance, validated_data):
        ...
        if choice not in valid_choices:
            raise ValidationError({
                "status":
                [f"Next valid choices are: {', '.join(valid_choices)}"]
            })
        ...
        if not instance.date_of_sample and validated_data.get("status") in [
                PatientSample.SAMPLE_TEST_FLOW_MAP[key] for key in [
            ...
            validated_data["result"] = PatientSample.SAMPLE_TEST_RESULT_MAP[
                "AWAITING"]
