...
def candidate_in_scope(candidate: str,
                       scope: List,
                       is_application: bool = False) -> bool:
    """
    ...
    for item in scope:
        ...
        if search.type == "regex":
            match = bool(
                re.match(search.value, search_candidate, re.IGNORECASE))
...
def exclude_model(model_name):
    ...
    if candidate_in_scope(model_name,
