...
from care.facility.models import (CATEGORY_CHOICES, DISEASE_CHOICES_MAP, SYMPTOM_CHOICES, Disease, DiseaseStatusEnum,
                                  Facility, LocalBody, PatientConsultation, PatientRegistration, User)
...
class TestBase(APITestCase):
    ...
    def create_state(cls):
        return State.objects.create(
            name=f"State{datetime.datetime.now().timestamp()}")
    ...
    def create_patient(cls, **kwargs):
        ...
        patient_data.update({
            ...
            state_id,
            "disease_status":
            getattr(DiseaseStatusEnum, patient_data["disease_status"]).value,
        })
    ...
    def get_local_body_district_state_representation(self, obj):
        ...
        response = {}
        response.update(
            self.get_local_body_representation(getattr(obj, "local_body",
                                                       None)))
        response.update(
            self.get_district_representation(getattr(obj, "district", None)))
        response.update(
            self.get_state_representation(getattr(obj, "state", None)))
        return response
    ...
    def assertDictEqual(self,
        ...
        return super(TestBase, self).assertDictEqual(first_dict, second_dict,
                                                     msg)
    ...
    def _convert_to_matchable_types(d):
        ...
        def to_matching_type(name: str, value):
            if isinstance(value, (OrderedDict, dict)):
                ...
                return (return_value.astimezone(
                    tz=datetime.timezone.utc) if isinstance(
                        return_value, datetime.datetime) else return_value)
            return value
    ...
    @classmethod
    def create_consultation(cls,
                            patient=None,
                            facility=None,
                            referred_to=None,
                            **kwargs) -> PatientConsultation:
